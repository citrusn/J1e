...
;
: kbfifo-fullness ( -- u ) 
    \ how many unread bits in the kbfifo
    kbfifocount @ kbread @ - h# ff and
;

variable ps2_clk' \ sds убрал
...
: kbraw ( -- u ) \ get the current raw keycode
    \ kbstate @ d# 2 rshift h# ff and \ sds
    kbnew \ sds
    kbread @ d# 255 and 
;
...
: rawloop1    
    begin
        kbfifocount @ lock ! \ sds
        kbfifo-fullness 0<>  \ sds
        rawready? 0= and
    while \ false идем на выход
